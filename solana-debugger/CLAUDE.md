...
| `lib/bisect.ts` | drops instructions the failure doesn't need (`whatif -- --bisect`) |
...
- **A perturbation has to be gated on a baseline that reproduced.** `lib/counterfactual.ts` and `lib/bisect.ts` both re-run a modified transaction and compare, which is only evidence about the transaction if the *unmodified* run first landed on mainnet's outcome. Both check that and report `reliable: false` rather than presenting fork behaviour as fact. `lib/bisect.ts` additionally compares failures by payload rather than by `InstructionError[0]`, since dropping an earlier instruction shifts the index while the error stays the same — comparing indexes would minimize nothing.
